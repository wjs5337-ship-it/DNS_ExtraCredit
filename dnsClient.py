...
def dns_query(type, name, server):
    ...
    if type == 'A':
        qtype = 1     # Lookup the Resource Record value - JS
    elif type == 'AAAA':
        qtype = 28     # Lookup the Resource Record value - JS
    else:
        raise ValueError('Invalid type')
    

    qclass = 1     # Lookup the Resource Record class being requested (JS: 1 = IN = Internet)
    ...
    for _ in range(ANCOUNT):
        ...
        offset += rdlength # Same value as just calculated
